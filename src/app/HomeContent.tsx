  ...
  useEffect(() => {
    ...
    function updateVideo() {
      ...
      if (media.matches && !connection?.saveData) {
        video.src = '/images/gallery/hero-video.mp4';
        void video.play().catch(() => { /* The poster remains when autoplay is unavailable. */ });
      } else {
        video.pause();
    ...
    return () => { media.removeEventListener('change', updateVideo); video?.pause(); };
  }, []);
  return (
    <>
      {/* HERO */}
      <section className="relative overflow-hidden">
        <video
          ref={videoRef}
          poster="/images/gallery/hero-poster.webp"
          preload="none"
          aria-hidden="true"
          muted
          loop
